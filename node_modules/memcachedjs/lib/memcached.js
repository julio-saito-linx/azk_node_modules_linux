...
function MemcachedData (data)
{
	...
	this.getNewBuffer = function(start, end) {
...
function MemcachedTCPDataHandler (memcached) {
	...
	this.process = function (data, callback) {
		...
		var commandList = new cmd.MemcachedCommandList();

		this.mdata = this.processData(this.mdata, commandList, callback);
	};

	this.processData = function(mdata, commandList, callback) {
		...
		var command = (commandList).getCommand(mdata);
		if (!command) return mdata;

		var parsed = command.tryParseData(mdata.data);
		if (!parsed) return mdata;
		...
		log.write(function(){ return "MemcachedTCPDataHandler.process: (command data processed)" +
			" cmd: " + command.commandName + " partialResult.length: " + partialResult.length }, 3);
		...
		var next = function() { this.processData(mdata, commandList, callback); }
		return process.nextTick(next.bind(this));
	};
	...
	this.getNextMemcachedData = function (command, mdata) {
		...
		if (command.packageCommandLength != mdata.data.length) {
			try{

				var start = command.packageCommandLength;
				var end = mdata.data.length - command.packageCommandLength;
				var b = mdata.getNewBuffer(start, end)
				...
				console.log("command.packageCommandLength: " + command.packageCommandLength);
